...
_Indeed if $a*x = b*x \mod p$ then since $F_p$ is a field, $x$ has a multiplicative inverse $y$ with $xy=1\mod p$,
so $a = a*1 = a*x*y = b*x*y =b*1 = b \mod p$ so $x\mapsto a*x$ is an injection from $F_p$ to itself and hence is a bijection._
