...
For example, if we let $n=2^k$ and we let $s_n$ be the number of comparisons needed to merge sort a list of size $n$,
...
$$(1-2x)f_s(x) = 1 + \frac 1 {1-2x}$$

so 

$$f_s(x) = \frac 1 {1-2x} + \frac 1 {(1-2x)^2} = \sum_{k=0}^\infty (2^k + (k+1)2^k)x^k$$

so $s_k = n + (\log{n} + 1) n$ where $n=2^k$ so $k = \log{n}$
