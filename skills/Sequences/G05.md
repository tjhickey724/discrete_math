...
For example, if we let $n=2^k$ and we let $s_n$ be the number of "steps" $s_n$ needed to merge sort a list of size $n$,

The algorithm takes a list of size $n=2^k$ and splits it into two sublists (the left and right halfs) of size $n/2 = 2^{k-1}$
