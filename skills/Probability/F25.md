...
_We can solve this by looking at an infinite tree of outcomes, calculating the probability of each one, and evaluating the resulting series.
...
$$
E = \sum_{n=1}^\infty (n+1)0.5^{n+1} = 0.5 * \sum_{n=1}^\infty (n+1)0.5^n
$$
...
$$ E = 0.5 \left (\frac 1 {(1-0.5)^2} - 1\right ) = 0.5 (\frac 1 {1/4}) = 0.5 * 4 = 2 $$
