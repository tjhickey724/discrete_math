...
$$
It is not hard to see that $P^k(N) \subset P^\omega$ for each $k$, so $P^\omega$ is bigger than all of them@
and we can continue this process with $P^k(P_\omega) = P^{\omega + k}(N)$ if we then take the union of all of those
we get $P^{2\omega)$ and continuing this process we get $P^{\omega * \omega}=P^{\omega^2}$, and $P^{\omega^k$,
and $P^{omega^\omega)$ and $P^{omage^{omega^\omega}}$ and  so on  
$$
P(N) \subset P^k(N) \subset P^\omega(N) \subset P^{\omega+k}(N) \subset P^{2\omega} \subset P^{\omega*\omega) 
\subset P^{\omega^\omega)  \subset 
P^{\omega^{\omage^{omega^\ldots}}}
$$
This the tower of cardinal numbers and the exponents are called the oridinal numbers. There is an large and fascinating theory of infinite numbers, but we won't have time to get into it here!
